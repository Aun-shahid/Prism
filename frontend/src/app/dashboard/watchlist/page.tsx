...
export default function WatchlistPage() {
  ...
  const [openManageTargetsDialog, setOpenManageTargetsDialog] = React.useState(false);
  ...
  const handleDeleteTarget = async (id: string) => {
    if (!confirm('Are you sure you want to remove this watchlist target? All discovered jobs for it will be deleted.')) return;
    try {
      await scraperService.deleteTarget(id);
      setSuccess('Watchlist target removed.');
      loadData();
    } catch (err: any) {
  ...
  const handleTriggerScrape = async (id: string) => {
    ...
    try {
      ...
      setError('Scan completed but failed to scrape. Check target URL access restrictions.');
      loadData();
    } finally {
      setScrapingId(null);
    }
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4, flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
            Define company career portals and public job search feeds to monitor open positions automatically.
          </Typography>
        </Box>
        <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}>
          <Button 
            variant="outlined" 
            onClick={() => setOpenManageTargetsDialog(true)}
            sx={{ 
              borderRadius: '8px', 
              fontWeight: 700, 
              textTransform: 'none',
          ...
          >
            Watchlist Targets ({targets.length})
          </Button>
          <Button 
            variant="contained" 
            startIcon={<AddIcon />} 
          ...
          {isAdmin && (
            <>
              <Button 
                variant="outlined" 
                onClick={() => setOpenManageFeedsDialog(true)}
                sx={{ 
                  borderRadius: '8px', 
                  fontWeight: 700, 
                  textTransform: 'none',
              ...
              <Button 
                variant="outlined" 
                startIcon={<AddIcon />} 
                onClick={() => setOpenAddGeneralDialog(true)}
                sx={{ 
                  borderRadius: '8px', 
                  fontWeight: 700, 
                  textTransform: 'none',
      ...
      {/* Discovered Positions Feed - FULL WIDTH */}
      <Box sx={{ width: '100%', mt: 1 }}>
        <Card sx={{ borderRadius: '12px' }}>
          <CardContent sx={{ p: 3.5 }}>
            <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 800 }}>Discovered Positions Feed</Typography>
            ...
            {loadingJobs ? (
              ...
              <Paper sx={{ p: 8, textAlign: 'center', color: 'text.secondary', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '8px' }}>
                ...
                <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 0.5 }}>
                  {targets.length === 0 ? "No company targets added yet" : "No matched positions yet"}
                </Typography>
                <Typography variant="body2" sx={{ display: 'block', maxWidth: 450, mx: 'auto', opacity: 0.7 }}>
                  {targets.length === 0 
                    ? "Add a company career page to monitor (like Systems Limited or any specific company) to automatically track open positions matching your keyword preferences."
                    : "Activate your watchlist targets and run manual scans from the \"Watchlist Targets\" manager, or wait for background scans to pick them up."}
                </Typography>
                {targets.length === 0 && (
                  <Button 
                    variant="contained" 
                    startIcon={<AddIcon />} 
                    onClick={() => setOpenAddDialog(true)}
                    sx={{ mt: 3, borderRadius: '8px', fontWeight: 700, textTransform: 'none' }}
                  >
                    Add Company Target
                  </Button>
                )}
      ...
      {/* Manage Watchlist Targets Dialog */}
      <Dialog 
        open={openManageTargetsDialog} 
        onClose={() => setOpenManageTargetsDialog(false)} 
        fullWidth 
        maxWidth="md"
      >
        <DialogTitle component="div" sx={{ fontWeight: 800, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6" component="div" sx={{ fontWeight: 800 }}>Watchlist Targets ({targets.length})</Typography>
          <Button 
            variant="contained" 
            startIcon={<AddIcon />} 
            size="small" 
            onClick={() => {
              setOpenManageTargetsDialog(false);
        ...
        <DialogContent dividers>
          {loadingTargets ? (
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary', textAlign: 'center', py: 4 }}>
              No watchlist targets configured. Add a website to monitor.
            </Typography>
          ) : (
            <List sx={{ pt: 1 }}>
              {targets.map((t) => (
        ...
        <DialogActions sx={{ p: 2.5 }}>
          <Button onClick={() => setOpenManageTargetsDialog(false)} variant="outlined">Close</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={openAddDialog} onClose={() => setOpenAddDialog(false)} fullWidth maxWidth="sm">
        <Box component="form" onSubmit={handleSaveTarget}>
          <DialogTitle sx={{ fontWeight: 800 }}>Add Watchlist Target</DialogTitle>
          <DialogContent dividers>
